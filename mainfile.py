...
title_label = tk.Label(win,text="Student Management System",font=("Arial",30,"bold"),border=12,relief=tk.GROOVE,bg="lightgreen",foreground="blue")
title_label.pack(side=tk.TOP,fill=tk.X)

detail_frame = tk.LabelFrame(win, text="Enter Detail", font=("Arial",20),bd=12, relief=tk.GROOVE,bg="lightgrey")
...
fathernm_lbl = tk.Label(detail_frame,text="Father's Name",font=("Arial",15),bg="lightgrey")
fathernm_lbl.grid(row=5,column=0,padx=2,pady=2)

fathernm_ent = tk.Entry(detail_frame,bd=7,font=("Arial",15),textvariable=fathernm)
fathernm_ent.grid(row=5,column=1,padx=2,pady=2)
...
def fetch_data():
    ...
    curr = conn.cursor()
    curr.execute("SELECT * FROM sdata")
    rows = curr.fetchall()
    if len(rows)!=0:
...
def add_func():
    if rollno.get()=="" or name.get()=="" or class_var.get()=="":
        ...
        conn = pymysql.connect(host="localhost",user="root",password="",database="sms1")
        curr = conn.cursor()
        curr.execute("INSERT INTO sdata VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(rollno.get(),name.get(),class_var.get(),section.get(),contact.get(),fathernm.get(),address.get(),gender.get(),dob.get()))
        conn.commit()
        conn.close()

        fetch_data() #----> fetch data after adding

def get_cursor(event):
    '''this function fetch data of the sekected row'''
...
def clear_func():
    '''this function clear the entry boxes'''
    rollno.set("") 
    name.set("")
    class_var.set("")
    section.set("")
...
def update_func():
    ...
    conn = pymysql.connect(host="localhost",user="root",password="",database="sms1")
    curr = conn.cursor()
    curr.execute("UPDATE sdata set name=%s,class=%s,section=%s,contact=%s,fathernm=%s,address=%s,gender=%s,dob=%s where rollno=%s",(name.get(),class_var.get(),section.get(),contact.get(),fathernm.get(),address.get(),gender.get(),dob.get(),rollno.get()))
    conn.commit()
    conn.close()
    fetch_data()
    clear_func()
...
def delete_func():
    '''this function will delete the selected data'''
    ...
    if confirmation:
        ...
        # Get the rollno from the selected item
    

        # Execute the delete query
        curr.execute("DELETE FROM sdata WHERE rollno=%s", (rollno.get()))
        conn.commit()
        conn.close()
        fetch_data()
        clear_func()
...
btn_frame.place(x=20,y=390,width=345,height=120)
...
update_btn.grid(row=0,column=1,padx=6,pady=2)
...
clear_btn = tk.Button(btn_frame,bg="lightgrey",text="Clear",bd=7,font=("Arial",13),width=15,command=clear_func)
clear_btn.grid(row=1,column=1,padx=6,pady=3)
